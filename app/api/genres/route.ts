...
export async function GET(request: NextRequest) {
  ...
  try {
    const cacheKey = buildCacheKey(CACHE_SCOPES.publicTMDb, 'genres:full');
    const data = await redisCache.getOrSet(cacheKey, async () => {
      ...
      const genresWithImages = await Promise.all(
        list.genres.map(async (genre: { id: number }) => {
      ...
      return { genres: genresWithImages };
    }, GENRES_TTL);
