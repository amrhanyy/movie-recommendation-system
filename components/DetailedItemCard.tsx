...
import { Bookmark, Star, Calendar, Clock, Film, Tv2, Info } from 'lucide-react';
...
export function DetailedItemCard({ item, onRemove }: DetailedItemCardProps) {
  ...
  return (
    <div 
      ...
      className="group flex gap-6 p-4 rounded-xl border border-gray-700/50 bg-gray-800/30 hover:bg-gray-800/50 
        transition-all duration-300 cursor-pointer hover:border-cyan-700/30 relative overflow-hidden"
    >
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-r from-cyan-950/5 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500"></div>
      
      {/* Poster */}
      <div className="relative h-40 w-28 flex-shrink-0 rounded-lg overflow-hidden border border-gray-700/50">
        {!imageLoaded && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900/80 z-10">
            <div className="w-6 h-6 border-2 border-gray-600 border-t-cyan-500 rounded-full animate-spin"></div>
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        <div className="flex justify-between items-start gap-4">
          <h3 className="text-lg font-medium text-white group-hover:text-cyan-300 transition-colors mr-auto">{item.title}</h3>
          
          <button
            onClick={handleRemoveClick}
            className="flex-shrink-0 p-2 rounded-full bg-black/40 hover:bg-black/60 
              text-cyan-500 border border-gray-700/50 hover:border-cyan-500/50 hover:text-cyan-400
        ...
        {item.voteAverage && (
          <div className="flex items-center gap-1 mb-2 mt-1">
            ...
            <span className="text-yellow-400 font-medium">{item.voteAverage.toFixed(1)}/10</span>
          </div>
        )}
